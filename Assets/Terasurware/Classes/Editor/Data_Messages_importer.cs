...
public class Data_Messages_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/DataSheet/Data_Messages.xlsx";
	private static readonly string exportPath = "Assets/DataSheet/Data_Messages.asset";
	...
	static void OnPostprocessAllAssets (string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		foreach (string asset in importedAssets) {
			...
			Data_Messages data = (Data_Messages)AssetDatabase.LoadAssetAtPath (exportPath, typeof(Data_Messages));
			if (data == null) {
				data = ScriptableObject.CreateInstance<Data_Messages> ();
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					Data_Messages.Sheet s = new Data_Messages.Sheet ();
					s.name = sheetName;
					...
					for (int i=1; i<= sheet.LastRowNum; i++) {
						...
						Data_Messages.Param p = new Data_Messages.Param ();
						
					cell = row.GetCell(0); p.Scene = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(1); p.DialogIndex = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(2); p.Name = (cell == null ? "" : cell.ToString());
					cell = row.GetCell(3); p.Actor = (int)(cell == null ? 0 : cell.NumericCellValue);
